...
public interface MuleServer extends SoftwareProcess, UsesJmx, HasShortName {
    ...
    @SetFromFlag("version")
    ConfigKey<String> SUGGESTED_VERSION = ConfigKeys.newConfigKeyWithDefault(SoftwareProcess.SUGGESTED_VERSION, "3.6.1");

    @SuppressWarnings({ "unchecked", "rawtypes" })
    public static final AttributeSensor<Set<String>> DEPLOYED_APPS = new BasicAttributeSensor(Set.class, "mule.deployedApps",
            "Names of apps that are currently in $MULE_HOME/apps");
     ...
     *
     * If deployment of targetName is successful, targetName is returned. This targetName can be used as an argument to
     * undeploy. If deployment is unsuccessful, an empty String is returned.
     * 
     * @param url
     *            where to get the zip file, as a URL, either classpath://xxx or file:///home/xxx or http(s)...
     * @param targetName
    ...
    /**
     * For the DEPLOYED_WARS to be updated, the input must match the result of the call to deploy, e.g. the transformed name
     * using
     */
