...
public class MuleSshDriver extends JavaSoftwareProcessSshDriver implements MuleDriver {
	...
	public boolean isRunning() {
		int res = newScript(MutableMap.of(USE_PID_FILE, false), CHECK_RUNNING).body.append(format("bin/mule status"))
				.execute();
		return res == 0;
	}
	...
	public String deploy(String url, String targetName) {
		...
		if (result != 0) {
			log.warn("Problem deploying {} to {}:{} for {}: result {}", new Object[] { url, getHostname(), dest,
					entity, result });
	...
	public void undeploy(String targetName) {
		...
		if (result != 0) {
			log.warn("Problem undeploying {} from {}:{} for {}: result {}", new Object[] { targetName, getHostname(),
					dest, entity, result });
